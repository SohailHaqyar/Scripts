...
[[ "${rofiScale}" =~ ^[0-9]+$ ]] || rofiScale=8
r_scale="configuration {font: \"OverpassM Nerd Font ${rofiScale}\";}"
...
case "${1}" in
c|-c|--copy)
    cliphist list | rofi -dmenu -theme-str "${r_scale}" | cliphist decode | wl-copy
    ;;
d|-d|--delete)
    cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"Delete...\";}" -theme-str "${r_scale}"  | cliphist delete
    ;;
w|-w|--wipe)
